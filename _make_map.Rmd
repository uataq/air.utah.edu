...
# Data read --------------------------------------------------------------------
raw <- readRDS('/srv/shiny-server/map/shared/airmap.rds')
dat <- raw$fixed
...
if (nrow(dat) > 1) {
  for (site in unique(dat$site)){
    ...
    mpop<- paste(sep='<br>',
                 paste0('<b>', pretty_site_names[dat$site], '</b>'),
                 paste(tracer, ':<b>', round(dat[[short_species[[tracer]]]], 2), '</b>', units[[tracer]]),
                 paste('Last updated:  ', format(dat$Time_UTC, '%Y-%m-%d %H:%M %Z')),
                 a(target='_blank',
                   href=url_link[dat$site],
                   tags$button(type='button',
                               class='btn action-button btn-large btn-danger',
                               HTML('<i class="fa fa-line-chart"></i>Go to the data!'))))
    
    leaf <- leaf %>%
      addCircleMarkers(lng=dat$lon, lat=dat$lat, radius=2, popup=mpop, stroke=T, weight=2,
                       fillColor=cpal(color), color=cpal(color),
                       opacity=0.3, fillOpacity=0.3) %>%
      addCircleMarkers(lng=tail(dat$lon, 1), lat=tail(dat$lat, 1), radius=5, popup=tail(mpop, 1),
                       stroke=T, weight=2, fillColor=cpal(tail(color, 1)), color='black',
