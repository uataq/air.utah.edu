...
### Network Diagnostics (1-day) <span style="float:right;"><a href="https://github.com/uataq/data-pipeline#qaqc-flagging-conventions"><i class="fa fa-bars"></i> QAQC Definitions</a></span>
```{r}
pd_ref <- data$qaqc %>%
...
list(
  ...
  data$qaqc %>%
    ...
    group_by(stid) %>%
    summarize(qaqc = paste(setdiff(unique(QAQC_Flag), c(-9, -1, 0)), collapse = ',')),
  data$qaqc %>% 
    filter(ID_CO2 == -10, 
           Time_UTC > Sys.time() - 86400) %>%
    group_by(stid) %>%
